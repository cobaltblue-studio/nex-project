  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-2 mb-1">
        <span className="text-[9px] font-bold tracking-[0.3em] text-primary/60 uppercase">{label}</span>
      ...
      <div>
        <p className="text-sm font-bold text-white truncate" data-testid={`text-battle-title-${track.id}`}>{track.title}</p>
        <p className="text-[10px] text-zinc-500 uppercase tracking-widest mt-0.5">{track.creatorName}</p>
      </div>
    </div>
  );
}
...
export function Battle() {
  ...
  const createBattleMutation = useMutation({
    mutationFn: (genre: string) => apiRequest("POST", "/api/battles/new", { genre }),
    ...
    onError: () => {
      toast({ title: "Could not start battle", description: "Not enough tracks in this genre.", variant: "destructive" });
      setPhase("genre-select");
    },
  });

  const voteMutation = useMutation({
    mutationFn: ({ battleId, trackId }: { battleId: number; trackId: number }) =>
    ...
    onError: (err: any) => {
      if (err?.message?.includes("409") || err?.status === 409) {
        toast({ title: "Already voted", description: "You can only vote once per battle.", variant: "destructive" });
      } else {
  ...
  const castVote = useCallback((trackId: number) => {
    if (!isAuthenticated) {
  ...
  const winnerTrack = voteResult && battle
    ? (voteResult.winnerId === battle.trackAId ? battle.trackA : battle.trackB)
    : null;
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="mb-10">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-[11px] font-bold tracking-[0.4em] uppercase text-primary">NEX Battle Arena</h1>
        </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {phase === "genre-select" && (
          ...
          >
            <p className="text-zinc-400 text-sm mb-6">Choose a genre to start a battle between two AI-generated tracks.</p>
            {genresLoading ? (
              <div className="flex gap-2 flex-wrap">
                {[1, 2, 3, 4].map(i => (
                  <div key={i} className="h-10 w-28 bg-white/5 rounded-sm animate-pulse" />
                ))}
              </div>
            ) : genres.length === 0 ? (
        ...
        {phase === "loading" && (
          ...
          >
            ...
            <p className="text-[11px] font-bold tracking-[0.3em] uppercase text-zinc-500">Loading Battle…</p>
          </motion.div>
        )}
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className={`border rounded-sm p-5 flex flex-col gap-4 transition-all ${listenedA && listenedB ? "border-white/10 bg-black/30" : "border-white/5 bg-black/15 opacity-60"}`}>
                ...
                <div>
                  <p className="font-bold text-white text-sm" data-testid="text-vote-track-a-title">{battle.trackA.title}</p>
                  <p className="text-[10px] text-zinc-600 uppercase tracking-widest mt-0.5">{battle.trackA.creatorName}</p>
                </div>
                <button
              ...
              <div className={`border rounded-sm p-5 flex flex-col gap-4 transition-all ${listenedA && listenedB ? "border-white/10 bg-black/30" : "border-white/5 bg-black/15 opacity-60"}`}>
                ...
                <div>
                  <p className="font-bold text-white text-sm" data-testid="text-vote-track-b-title">{battle.trackB.title}</p>
                  <p className="text-[10px] text-zinc-600 uppercase tracking-widest mt-0.5">{battle.trackB.creatorName}</p>
                </div>
                <button
            ...
            {!isAuthenticated && (
              <p className="text-center text-zinc-600 text-[10px] uppercase tracking-widest mt-4">
                <a href="/api/login" className="text-primary hover:underline">Login</a> to cast your vote
              </p>
            )}
          </motion.div>
        )}
        ...
        {/* Phase: Result */}
        {phase === "result" && battle && voteResult && winnerTrack && (() => {
          const total = voteResult.trackAVotes + voteResult.trackBVotes;
          const pctA = total > 0 ? Math.round((voteResult.trackAVotes / total) * 100) : 50;
          const pctB = total > 0 ? Math.round((voteResult.trackBVotes / total) * 100) : 50;
          ...
          return (
            ...
            >
              ...
              <div>
                <p className="text-[10px] font-bold tracking-[0.5em] uppercase text-zinc-500 mb-2">Winner</p>
                <h3
                ...
                <p className="text-zinc-500 text-sm mt-1 uppercase tracking-widest">{winnerTrack.creatorName}</p>
              </div>
              ...
              <div className="w-full">
                <div className="flex items-stretch gap-0 w-full mb-3">
                  ...
                  <div
                    ...
                    style={{ width: `${pctA}%`, minWidth: "30%", background: winnerIsA ? "rgba(0,240,255,0.08)" : "rgba(255,255,255,0.03)", borderLeft: "1px solid rgba(255,255,255,0.08)", borderTop: "1px solid rgba(255,255,255,0.08)", borderBottom: "1px solid rgba(255,255,255,0.08)" }}
                  >
                    <p className={`text-2xl font-display font-bold ${winnerIsA ? "text-primary" : "text-zinc-400"}`} data-testid="text-pct-a">
                      {pctA}%
                    </p>
                    <p className={`text-[11px] font-bold mt-0.5 ${winnerIsA ? "text-primary/70" : "text-zinc-600"}`} data-testid="text-votes-a">
                      {voteResult.trackAVotes} vote{voteResult.trackAVotes !== 1 ? "s" : ""}
                    </p>
                    <p className="text-[8px] uppercase tracking-widest text-zinc-600 mt-1 truncate max-w-[90px]">{battle.trackA.title}</p>
                  </div>

                  {/* Track B bar */}
                  <div
                    className="flex flex-col items-center justify-center py-4 px-3 transition-all"
                    style={{ width: `${pctB}%`, minWidth: "30%", background: !winnerIsA ? "rgba(96,165,250,0.08)" : "rgba(255,255,255,0.03)", borderRight: "1px solid rgba(255,255,255,0.08)", borderTop: "1px solid rgba(255,255,255,0.08)", borderBottom: "1px solid rgba(255,255,255,0.08)" }}
                  >
                    <p className={`text-2xl font-display font-bold ${!winnerIsA ? "text-blue-400" : "text-zinc-400"}`} data-testid="text-pct-b">
                      {pctB}%
                    </p>
                    <p className={`text-[11px] font-bold mt-0.5 ${!winnerIsA ? "text-blue-400/70" : "text-zinc-600"}`} data-testid="text-votes-b">
                      {voteResult.trackBVotes} vote{voteResult.trackBVotes !== 1 ? "s" : ""}
                    </p>
                    <p className="text-[8px] uppercase tracking-widest text-zinc-600 mt-1 truncate max-w-[90px]">{battle.trackB.title}</p>
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-3 w-full max-w-sm">
                ...
                <button
                  onClick={() => { setBattle(null); setVoteResult(null); setSelectedGenre(""); setPhase("genre-select"); }}
