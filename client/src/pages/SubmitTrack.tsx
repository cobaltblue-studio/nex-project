...
import { Send, CheckCircle, Music, Link, ChevronDown, User, Tag } from "lucide-react";
import { useAuth } from "@/hooks/use-auth";

const GENRES = ["Electronic", "Synth Pop", "Rock", "Hip Hop", "Ambient", "Other"] as const;
...
const schema = z.object({
  title: z.string().min(1, "Track title is required").max(120, "Title too long"),
  artistName: z.string().min(1, "Creator name is required").max(80, "Name too long"),
...
export default function SubmitTrack() {
  ...
  const detectedPlatform = (() => {
    ...
    if (!link) return null;
    return SUPPORTED_LINKS.find(({ pattern }) => pattern.test(link))?.name ?? null;
  })();
              ...
              {!isAuthenticated && (
                <div className="border border-yellow-500/20 bg-yellow-500/5 rounded-sm p-4 mb-6 text-center">
                  <p className="text-[10px] text-yellow-400/80 uppercase tracking-widest">
                    <a href="/api/login" className="text-yellow-400 hover:underline font-bold">
                      Login
                    </a>{" "}
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                ...
                {mutation.isError && (
                  <p className="text-[10px] text-red-400 text-center uppercase tracking-widest">
                    {(mutation.error as Error)?.message || "Submission failed. Try again."}
                  </p>
                )}
                ...
                <div className="border border-white/5 rounded-sm p-4 bg-white/2 text-[9px] text-zinc-600 uppercase tracking-widest space-y-1.5">
                  ...
                  <p>④ Chart: max 100 tracks ranked by votes + plays + battle wins</p>
                </div>
              </form>
            </motion.div>
          )}
